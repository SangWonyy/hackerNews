...
function newsDetail() {
  ...
  function makeCommment(comments) {
    ...
    for (var _i = 0; _i < comments.length; _i++) {
      ...
      if (comments[_i].comments.length > 0) {
        commentString.push(makeCommment(comments[_i].comments, called + 1));
      }
    }
  ...
  container.innerHTML = template.replace('{{__comments__}}', makeCommment(newsContent.comments));
}
